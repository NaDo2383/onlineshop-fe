...
export default function AdminProducts() {
  ...
  function fillCatArr(para, isChecked) {
    console.log("checked", isChecked, "para", para);
    console.log(productCat);
    if (isChecked) {
      productCat.push(para);
    } else {
      productCat.splice(productCat.indexOf(para), 1);
    }

    setAddingProduct({ ...addingProduct, categoryId: productCat });
  }
            ...
            return (
              <div className="d-flex gap-2 modal-checkboxes">
                <input
                  ...
                  id={e.id}
                  onChange={(a) => {
                    fillCatArr(e.id, a.target.checked);
                    console.log(productCat);
                  }}
                />
